...
describe("getPrefixPathname", () => {
  it("should extract prefix with single URL and single base", () => {
    expect(getPrefixPathname("/settings/profile", "/settings")).toBe("/settings");
    expect(getPrefixPathname("/admin/dashboard", "/admin")).toBe("/admin");
  });

  it("should extract prefix with single URL and multiple bases", () => {
    expect(getPrefixPathname("/settings/profile", ["/settings", "/admin"])).toBe(
      "/settings"
    );
    expect(getPrefixPathname("/admin/dashboard", ["/settings", "/admin"])).toBe("/admin");
  });

  it("should return null if URL does not match any base", () => {
    expect(getPrefixPathname("/other-path/profile", "/settings")).toBe(null);
    expect(getPrefixPathname("/other-path/profile", ["/settings", "/admin"])).toBe(null);
  });
  ...
  it("should autodetect prefix with levels > 1", () => {
    expect(getPrefixPathname("/settings/profile/info", null, { levels: 2 })).toBe(
      "/settings/profile"
    );
  });
  ...
  it("should process multiple URLs and single base", () => {
    expect(
      getPrefixPathname(["/settings/profile", "/settings/password"], "/settings")
    ).toEqual("/settings");
  });

  it("should process multiple URLs and multiple bases", () => {
    expect(
      getPrefixPathname(["/settings/profile", "/admin/password"], ["/settings", "/admin"])
  ...
  it("should respect levels in auto-detection for multiple URLs", () => {
    expect(
      getPrefixPathname(["/settings/profile/info", "/admin/dashboard/logs"], null, {
        levels: 2
      })
  ...
  it("should normalize duplicate-looking paths", () => {
    ...
    expect(getPrefixPathname(["/settings//profile", "/settings///password"])).toEqual(
      "/settings"
    );
  });
  ...
  it("should throw TypeError for invalid parameters", () => {
    ...
    expect(() => getPrefixPathname("/settings", {} as any, "not object" as any)).toThrow(
      TypeError
    );
    expect(() =>
  ...
  it("should support explicit auto-detection with no base and options", () => {
    expect(getPrefixPathname("/foo/bar/baz", null, { levels: 2 })).toBe("/foo/bar");
  });

  it("should support array input with no base and options", () => {
    expect(getPrefixPathname(["/foo/bar", "/foo/baz"], null, { levels: 1 })).toBe("/foo");
  });
});
