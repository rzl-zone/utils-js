...
describe("formatEnvPort", () => {
  ...
  it("should throw if prefixColon is not boolean", () => {
    expect(() => formatEnvPort("8080", { prefixColon: "yes" as any })).toThrow(TypeError);
    expect(() => formatEnvPort("8080", { prefixColon: 1 as any })).toThrow(TypeError);
    expect(() => formatEnvPort("8080", { prefixColon: null as any })).toThrow(TypeError);
  });
  ...
  it("should handle complex strings with multiple colons and symbols", () => {
    ...
    expect(formatEnvPort("path/to/port:9999", { prefixColon: true })).toBe(":9999");
  });
