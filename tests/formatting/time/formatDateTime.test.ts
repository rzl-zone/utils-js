...
import { formatDateTime } from "@/formatting/time";
...
describe("formatDateTime", () => {
  ...
  it("should handle custom format correctly", () => {
    ...
    expect(formatDateTime(dateSample, "YYYY/MM/DD hh-mm-ss")).toBe("2023/07/01 14-30-45");
    expect(formatDateTime(dateSample, "hh:mm")).toBe("14:30");
  });

  it("should not replace substrings that don't exist", () => {
    expect(formatDateTime(dateSample, "DATE: YYYY.MM.DD")).toBe("DATE: 2023.07.01");
