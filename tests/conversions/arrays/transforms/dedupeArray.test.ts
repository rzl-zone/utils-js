...
describe("dedupeArray with options object", () => {
    ...
    it("test 2", () => {
      ...
      const input = [
        [
          true,
          false,
          [null, date, [NaN, regex, set, obj, set, regex, [asyncFunc], asyncFunc]],
          undefined
        ],
        1,
        "1",
  ...
  it("should handle deeply mixed structures with forceToString: 'primitives' and flatten: true", () => {
    ...
    const input = [
      [true, false, [null, date, [NaN, regex, set, set, obj, [asyncFunc]]], undefined],
      1,
      "1",
      fn,
      [2, "2", fn, [3, "3", fn]]
  ...
  it("should handle deeply mixed structures with forceToString: 'stringOrNumber' and flatten: false", () => {
    ...
    expect(result).toEqual([
      [true, false, [null, date, [NaN, regex, set, objString, [asyncFunc]]], undefined],
      "1",
      fn,
      ["2", fn, ["3", fn]]
    ]);
  ...
  it("should handle deeply mixed structures with forceToString: false and flatten: false", () => {
    ...
    const input = [
      [true, false, [null, date, [NaN, regex, set, obj, [asyncFunc]]], undefined],
      1,
      "1",
      fn,
      [2, "2", fn, [3, "3", fn]]
  ...
  describe("type errors", () => {
    ...
    it("should throw TypeError if forceToString is an invalid value", () => {
      expect(() => dedupeArray([1, 2, 3], { forceToString: "invalid" as any })).toThrow(
        TypeError
      );
      expect(() => dedupeArray([1, 2, 3], { forceToString: 123 as any })).toThrow(
        TypeError
      );
      expect(() => dedupeArray([1, 2, 3], { forceToString: [] as any })).toThrow(
        TypeError
      );
    });
  });
