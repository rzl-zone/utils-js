...
describe("toBooleanExplicit", () => {
  ...
  it("should correctly detect truthy string 'indeterminate' values with options true", () => {
    expect(toBooleanExplicit("indeterminate", { includeIndeterminate: true })).toBe(true);
  });
  ...
  it("should throw if caseInsensitive is not a boolean", () => {
    expect(() => toBooleanExplicit("yes", { caseInsensitive: null as any })).toThrow(
      TypeError
    );

    expect(() => toBooleanExplicit("yes", { caseInsensitive: "abc" as any })).toThrow(
      TypeError
    );
  });
  ...
  it("should throw if trimString, caseInsensitive, includeIndeterminate is not a boolean", () => {
    ...
    expect(() => toBooleanExplicit("yes", { caseInsensitive: 123 as any })).toThrow(
      TypeError
    );
