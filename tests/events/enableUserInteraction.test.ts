...
import { enableUserInteraction } from "@/events/actions";
...
describe("enableUserInteraction", () => {
  ...
  it("should remove default class 'on_processing' from document.documentElement", () => {
    ...
    expect(document.documentElement.classList.contains("on_processing")).toBe(false);
  });
  ...
  it("should do nothing if the class does not exist", () => {
    ...
    expect(document.documentElement.classList.contains("not_present")).toBe(false);
  });
