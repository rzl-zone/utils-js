...
describe("textContainsAll - comprehensive tests", () => {
  it("should return true if all words are found in the text (default options)", () => {
    expect(textContainsAll("Hello world, WithAI APP", ["Hello", "world"], {})).toBe(true);
    expect(textContainsAll("JavaScript and TypeScript", ["Java", "Script"], {})).toBe(
      true
    );
  });

  it("should return false if any word is missing", () => {
    expect(textContainsAll("Machine Learning", ["AI", "Learning"], {})).toBe(false);
    expect(textContainsAll("Hello there", ["Hello", "world"], {})).toBe(false);
  });

  it("should respect exactMatch option", () => {
    expect(textContainsAll("open-source", ["open"], { exactMatch: true })).toBe(false);
    expect(textContainsAll("open source", ["open"], { exactMatch: true })).toBe(true);
  });

  it("should work with custom regex flags", () => {
    expect(textContainsAll("Hello WORLD", ["world"], { flags: "" })).toBe(false); // case-sensitive
    expect(textContainsAll("Hello WORLD", ["world"], { flags: "i" })).toBe(true); // case-insensitive
  });
  ...
  it("should return true with multiple spaces and line breaks", () => {
    expect(textContainsAll("Hello    world\nNew Line", ["Hello", "world"], {})).toBe(
      true
    );
  });

  it("should return true with partial words if exactMatch is false", () => {
    expect(textContainsAll("Javascript", ["Script"], { exactMatch: false })).toBe(true);
  });

  it("should return false with partial words if exactMatch is true", () => {
    expect(textContainsAll("Javascript", ["Script"], { exactMatch: true })).toBe(false);
  });

  it("should handle complex combination of special characters and spaces", () => {
    expect(textContainsAll("Price: 12,000.50 USD", ["12,000.50"], {})).toBe(true);
    expect(
      textContainsAll("Math formula: (a+b)^2 = a^2 + 2ab + b^2", ["(a+b)^2"], {})
    ).toBe(true);
  });
  ...
  it("should match words adjacent to punctuation with exactMatch false", () => {
    expect(textContainsAll("Hello, world!", ["world"], { exactMatch: false })).toBe(true);
    expect(textContainsAll("Hello, world!", ["world"], { exactMatch: true })).toBe(false);
  });

  it("should be able to handle tabs and multiple lines", () => {
    expect(textContainsAll("Hello\tworld\nnext line", ["Hello", "world"], {})).toBe(true);
  });

  it("should correctly return false when words are scrambled order", () => {
    expect(textContainsAll("Hello world test string", ["test", "world", "foo"], {})).toBe(
      false
    );
  });
