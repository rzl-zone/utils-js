...
import { randomIntByLength } from "@/generator/random/integer/randomIntByLength";
...
describe("randomIntByLength", () => {
  ...
  it("should throw an error for invalid minLength or maxLength values", () => {
    expect(() => randomIntByLength({ minLength: 0, maxLength: 2 })).toThrow(Error);
    expect(() => randomIntByLength({ minLength: 1, maxLength: 20 })).toThrow(Error);
    expect(() => randomIntByLength({ minLength: 5, maxLength: 2 })).toThrow(Error);
    expect(() => randomIntByLength({ minLength: 1.5, maxLength: 4 })).toThrow(Error);
  });
