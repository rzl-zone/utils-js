import { capitalizeWords } from "@/strings/case";
import { describe, it, expect } from "vitest";

describe("capitalizeWords with options", () => {
  it("should capitalize normally without options", () => {
    expect(capitalizeWords("hello world")).toBe("Hello World");
    expect(capitalizeWords("  hello   world  ", { trim: true })).toBe("Hello   World");
  });
  ...
  it("should collapse multiple spaces into one if collapseSpaces is true", () => {
    ...
    expect(capitalizeWords("this    is   a test", { collapseSpaces: true })).toBe(
      "This Is A Test"
    );
  });
  ...
  it("should handle strings with numbers and special chars", () => {
    expect(capitalizeWords("123 abc", { collapseSpaces: true })).toBe("123 Abc");
    expect(capitalizeWords("!hello @world", { trim: true })).toBe("!hello @world");
  });
});
...
describe("capitalizeWords with options", () => {
  it("should capitalize normally without options on long sentences", () => {
    ...
    expect(capitalizeWords(" another   example    with    excessive    spaces ")).toBe(
      " Another   Example    With    Excessive    Spaces "
    );
  });
  ...
  it("should handle edge case of only special characters", () => {
    ...
    expect(capitalizeWords(" @#$%^&*()_+ ", { trim: true, collapseSpaces: true })).toBe(
      "@#$%^&*()_+"
    );
  });
});
