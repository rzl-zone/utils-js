...
describe("extractFileName", () => {
  it("should extract file name without extension from URL", () => {
    expect(extractFileName("https://example.com/path/to/file.txt")).toBe("file");
  });
  ...
  it("should extract file name without multiple extensions", () => {
    ...
    expect(extractFileName("backup.2024-pertama.sql.gz")).toBe("backup.2024-pertama");
  ...
  it("should ignore non-matching extensions and keep full", () => {
    ...
    expect(extractFileName("odd.file.name.qwerty")).toBe("odd.file.name.qwerty");
  });
